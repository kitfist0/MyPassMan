...
class BackupManager(private val recordDao: RecordDao) {

    suspend fun exportDatabase(outputStream: OutputStream, password: CharArray) {
    ...
    suspend fun importDatabase(inputStream: InputStream, password: CharArray) {
        val encryptedData = inputStream.use { it.readBytes() }
        importDatabase(encryptedData, password)
    }

    suspend fun importDatabase(encryptedData: ByteArray, password: CharArray) {
